...
class SmartCashElectrumX(ElectrumX):
    '''A TCP server that handles incoming Electrum SmartCash connections.'''
    ...
    # Masternode command handlers
    async def masternode_announce_broadcast(self, signmnb):
        '''Pass through the smartnode announce message to be broadcast
        by the daemon.'''
        try:
            return await self.daemon.masternode_broadcast(['relay', signmnb])
        except DaemonError as e:
            error, = e.args
            message = error['message']
            self.log_info('smartnode_broadcast: {}'.format(message))
            raise RPCError('the smartnode broadcast was rejected.'
                           '\n\n{}\n[{}]'.format(message, signmnb))
